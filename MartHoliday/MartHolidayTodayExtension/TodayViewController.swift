    ...
    private func setFavoriteBranch(handler: @escaping ((Bool) -> Void)) {
        ...
        session.dataTask(with: url) { [weak self] (data, response, error) in
            if let response = response as? HTTPURLResponse, 200...299 ~= response.statusCode, let data = data {
                ...
                do {
                    branches = try JSONDecoder().decodeWithKeypath([BranchRawData].self, from: data, keyPath: "data")
...
extension JSONDecoder {
    func decodeWithKeypath<T: Decodable>(_ type: T.Type, from data: Data, keyPath: String) throws -> T {
